...
cmd_process()
{
	...
	if (strcmp("xhcireset", strcmd) == 0) {
		uart_puts("command XHCIRESET\n");
		mailbox_notify_xhci_reset();
	}
	if (strcmp("pcieinit", strcmd) == 0) {
		uart_puts("command PCIEINIT\n");
		pcie_init();
	}
	
	if (strcmp("pcieread", strcmd) == 0) {
		uint32_t index = hex2value(arg0);
		uint32_t addr = hex2value(arg1);
		uart_name_dword("PCIE :INDEX :  ", index);
		uart_name_dword("PCIE :ADDR  :  ", addr);
		uint32_t value = pcie_read(index, addr);
		uart_name_dword("PCIE :VALUE : ", value);
	}
	...
	if (cmd == 'r' || cmd == 'R') {
		...
		uart_name_dword("READ:ADDR : ", addr);
		uart_name_dword("READ:VALUE: ", value);
	}
	...
	if (cmd == 'w' || cmd == 'W') {
		...
		uart_name_dword("WRITE:ADDR : ", addr);
		uart_name_dword("WRITE:VALUE: ", value);
		IO_WRITE(addr, value);
	}
	...
	mailbox_fb_init(Width, Height);
	{
		...
		for (int y = 0 ; y < Height; y++)
			for (int x = 0 ; x < Width; x++)
				p[x + y * Width] = ( (x + count) ^ y);
	}
	*/
	#define POWER_ID_USB 3
	#define POWER_VALUE_ON 3
	mailbox_set_power_state(POWER_ID_USB, POWER_VALUE_ON);
