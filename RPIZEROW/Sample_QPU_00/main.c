...
extern uint32_t v3d_vertex_buffer[];

uint32_t test_copy_shader[] = {
#include "copy.h"
};
...
int notmain(void) {
	...
	{
		memcpy(v3d_test_shader, test_copy_shader, sizeof(test_copy_shader));
		memset(v3d_vertex_buffer, 0, 0x100);
		v3d_vertex_buffer[0] = 1.0f;
		v3d_vertex_buffer[1] = 2.0f;
		v3d_vertex_buffer[2] = 3.0f;
		v3d_vertex_buffer[3] = 4.0f;
		InvalidateData();
		//*V3D_VPMBASE = (uint32_t)v3d_vertex_buffer + VCADDR_BASE;
		*V3D_L2CACTL |= 1 << 0;
		*V3D_L2CACTL |= 1 << 2;
		*V3D_VPMBASE = (uint32_t)v3d_vertex_buffer;
		*V3D_SRQPC = (uint32_t)v3d_test_shader;
		
		while(1) {
			uart_dump((uint32_t)v3d_vertex_buffer, 0x200);
			uart_debug_puts("end : V3D_SRQCS  =", *V3D_SRQCS);
			uart_debug_puts("end : V3D_SQRSV0 =", *V3D_SQRSV0);
			uart_debug_puts("end : V3D_SQRSV1 =", *V3D_SQRSV1);
			uart_debug_puts("end : V3D_L2CACTL =", *V3D_L2CACTL);
			v3d_debug_print();
			for(int i = 0 ; i < 60; i++) {
				fake_vsync();
			}
