...
int usb_input_init(void) {
	ctrl_req.out_buffer = (uint8_t *)heap_4k_get();
	ctrl_req.in_buffer = (uint8_t *)heap_4k_get();
	ctrl_req.out_ch = CH_CTRL_OUT;
	ctrl_req.in_ch = CH_CTRL_IN;

	trans_intr.ch = CH_INTR_IN;
	trans_intr.buffer = (uint8_t *)heap_4k_get();
...
void usb_input_update() {
	...
	if(trans_intr.submit_count == 0) {
		trans_intr.ch = 3;
		trans_intr.len = 0x5;
		trans_intr.dev_addr = ADDR;
		trans_intr.epnum = 1;
		trans_intr.isin = 0;
		uint8_t *p = trans_intr.buffer;
		p[0] = 0x05;
		p[1] = 0x20;
		p[2] = 0x00;
		p[3] = 0x0f;
