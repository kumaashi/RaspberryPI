	...
	while(1)
	{
		...
		*USB_HCINT(2) |= 0x7FF;
		...
		//viz
		{
			...
			static int offset = 0;
			uint32_t *cptr = (uint32_t *)fb->pointer;
			for(int y = offset & 1 ; y < fb->height; y += 2) {
				for(int x = 0 ; x < fb->width; x++) {
					cptr[x + y * fb->width] >>= 1;
					if(x < 20 && y < ppad->Left_trigger) {
						cptr[x + y * fb->width] = 0xFF887755;
					}
					if(x > 220 && y < ppad->Right_trigger) {
						cptr[x + y * fb->width] = 0xFF887755;
					}
					if(y > 130) {
						cptr[x + y * fb->width] = pd[(x * 20) / WIDTH] << 7;
					}
				}
			}
			offset++;
